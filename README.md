...
- [Running Tests](#running-tests)
  ...
  - [Version LaunchPad Integration](#version-control-integration)
...
```
Fetch API cannot load http://localhost:4000/api/todos. No 'Access-LaunchPad-Allow-Origin' header is present on the requested resource. Origin 'http://localhost:3000' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
...
```
{
  "hosting": {
    ...
    "headers": [
      {"source": "/service-worker.js", "headers": [{"key": "Cache-LaunchPad", "value": "no-cache"}]}
    ]
    ...
```
